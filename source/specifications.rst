...
次の 49 通りです。「実行されるステップ」は、その組み合わせを選んだときに順に実行される
計算です。
...
すべての Job につきまして、Thread 数を増やすと計算時間が長くなる場合がございますのでご注意ください。

**Ver.7.0.0 では、交換相関汎関数で HSE06 の指定ができなくなっています。**

**コリニア磁性 / ノンコリニア磁性計算での初期スピンは、Quantum ESPRESSO の場合、原則として元素ごとにしか設定できません。もし同じ元素で、原子ごとに異なる値を設定した場合には、その平均値が採用されるため、意図しない設定になってしまうおそれがありますのでご注意ください。**
...
X線吸収スペクトル（XSpectra）では、下記の点にご注意ください。

-   **Ver.6.1.1 以前の入力ファイルを読み込むと、吸収サイトの情報が正しく反映されませんので、計算を実行する場合には Ver.6.1.2 以降で新しく Job を作成してください。**

-   格子ベクトル間角度が 90° ではないマテリアルでは、X-Spectra の四重極計算をデフォルト設定のまま実行すると、X 線 k ベクトルと X 線偏極ベクトルとの直交性が満たされず、異常終了してしまいます。

    X 線 k ベクトルと X 線偏極ベクトルの各成分は、Cartesian 座標基準ではなく、マテリアルの格子ベクトル (**a**\ :sub:`1`\, **a**\ :sub:`2`\, **a**\ :sub:`3`\) の係数 (*n*\ :sub:`1`\, *n*\ :sub:`2`\, *n*\ :sub:`3`\) として解釈されます。そのため、

    -   X 線 k ベクトル：(0 0 1)
    -   X 線偏極ベクトル：(1 0 0)

    というデフォルト設定では、この２ベクトルはそれぞれ **a**\ :sub:`3`\, **a**\ :sub:`1`\ と同一になります。例えば Si (mp-149) の構造ですと、この２ベクトルのなす角が 60° なので、計算を実行すると

    ::

         ERROR: xkvec and xepsilon are not orthogonal

    というエラーが出てしまいます。このようなケースでは、２ベクトルの直交性が満たされるよう、計算実行前にご自身で設定変更をお願いいたします。
...
**また、Thread 数を増やし過ぎた結果、エラーが出てしまう場合もございますので、ご自身で適切な Thread 数を設定してください。**
...
計算 Job 登録時に原子間ポテンシャルファイルをご自身でアップロードする場合、アップロード可能なファイル形式（拡張子）が下記に限定されておりますのでご注意ください。
