...
その前にまず、画面左上の「Job」メニューで Job を選択する必要があります。

.. image:: images/screenshot_0066.png

例として、「Si Electron Band Structure」を選択すると、Job の設定が表示され、
...
左サイドメニューの「File」を選択すると、各ジョブの入力ファイル（原子構造ファイル）、
出力ファイルの確認およびダウンロードが可能です。

.. image:: images/screenshot_0070.png

また、ファイル名をクリックすると、テキストファイルであれば、その内容を確認することができ、さらに編集も可能です。
この機能を用いて、GUIで提供していない、各計算ソフトの設定を行うことが可能です。
