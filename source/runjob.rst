...
**GPU 区分を選べるかどうかは、実行するソフトウェアによって異なります。** 対応していない
ソフトウェアでは GPU タブ自体が押せない状態になり、タブにカーソルを合わせると
「この計算ソフトウェアはGPU区分に対応していません」と表示されます。

対応している場合も、選べる GPU 数はソフトウェアによって 1 基だけに絞られることが
あります。選べない枚数の列は押せない状態になり、絞られている場合は表の下に選べる
枚数が注記されます。
...
.. note::
   ...
   **Job 詳細画面を開いている間、Status が「Submitted」「Prepared」「Running」の
   いずれかであれば、この画面は約 20 秒ごとに自動で更新されます。** 「ジョブ概要」の
   Status・計算ログ・ファイル一覧のいずれも、画面を再読み込みしなくても最新の状態に
   変わります。自動更新は、この画面をタブで表示している間だけ働き、他のタブに
   切り替えている間は止まります。

   **一覧画面（プロジェクトのジョブ一覧、ホーム画面のジョブ一覧など）は自動では
   更新されません。** Status の変化を確認するには、画面を再読み込みしてください。

計算ログは Job 詳細画面の「ログ」で確認できます。**実行中の Job では、画面を
表示している間、ログも約 20 秒ごとに自動で末尾が更新されます。** ログの中身の
見かたは :doc:`files` の章で説明します。
...
OpenMX と HybMD の Job に限り、終了した計算の続きから実行し直せます。継続元にできる
Status はソフトウェアによって異なります。
...
上の Status に当てはまる Job には、画面右上に「Continue Job」が表示されます。

-   押すと「Run Job」と同じダイアログが開き、計算リソースと実行時間の上限を
    選び直せます
-   元の Job の実行結果を引き継ぐため、**入力条件は変更できません**
-   継続して実行した Job は別の Job として作られ、Job 詳細画面の「継続履歴」に
    継続元との関係が表示されます

HybMD では、正常終了した MD 計算をさらに継続して延長する使い方もできます。
