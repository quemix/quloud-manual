...
**※ Quantum ESPRESSO では、初期スピンは原則として元素ごとにしか設定できません。もし同じ元素で、原子ごとに異なる値を設定した場合には、その平均値が採用されるため、意図しない設定になってしまうおそれがあります。**

**※ FLARE (On-the-Fly MD) では、spin 項目での各原子の初期スピンの設定、および constraint 項目での各原子の拘束条件の設定が無効となりますので、ご了承ください。**
...
**※ 現在、Property 選択欄で initial 以外の構造が正しく指定できない場合がございます。**
