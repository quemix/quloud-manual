...
.. list-table::
   ...
   * - 交換結合パラメータ
     - OpenMX
     - 原子間距離と交換結合定数（Jij）の関係を示すグラフ
   * - モンテカルロ磁性計算
     - Quloud-Mag
     - 磁化・帯磁率・エントロピー・全エネルギーの温度依存性グラフ
   * - LLGダイナミクス
     - Quloud-Mag
     - 磁化・エネルギー内訳の時間発展グラフ

モンテカルロ磁性計算・LLGダイナミクスのグラフには、表示する物理量と、その内訳（成分）を
切り替える 2 段のプルダウンがあります。データ点数が多い場合は間引いて表示され、その旨が
グラフの上に注記されます。
...
GROMACS と HybMD の MD 計算では、温度・全エネルギー・ポテンシャルエネルギー・運動
エネルギーなどの時間発展と、原子構造トラジェクトリを確認できます。HybMD の自己無撞着
電子状態計算のステップには、数値の一覧として表示される結果はありません。

RadonPy では、高分子平衡化MDで密度の時間発展とトラジェクトリを確認できます。それ以外の
高分子物性計算は、専用の結果カードを表示しません。出力ファイルを確認する方法は
:doc:`files` の章をご参照ください。

Psi4 では、自己無撞着電子状態計算（SCF）の結果に軌道エネルギー、HOMO-LUMO ギャップ、
双極子モーメントが表示されます。分子振動解析では、熱力学量と振動モードの一覧が表示されます。
構造最適化で得られた構造は、構造のカードから確認できます。
...
-   **NEB（Nudged Elastic Band）が収束条件を満たさないままステップ数の上限に達した場合、
    「NEB（反応経路）」カードに警告が表示されます。** 表示されている活性化エネルギー等の値は
    参考値になります。
